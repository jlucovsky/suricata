...
use nom7::bytes::complete::{tag, take_while1, take_until};
...
#[no_mangle]
pub unsafe extern "C" fn SCFTPPortFromPortResponse(input: *const u8, len: u32, dport_out: *mut u16) -> *mut c_char {
    if input.is_null() {
...
#[no_mangle]
pub unsafe extern "C" fn SCFTPPortFromPasvResponse(input: *const u8, len: u32, dport_out: *mut u16) -> *mut c_char {
...
fn parse_ftp_port_v6<'a>(i: &'a [u8], prefix: &[&str], suffix: Option<&str>) -> IResult<&'a [u8], (u16, &'a str)> {
    let mut input = i;
    ...
    let port_num: u16 = port_str.parse().map_err(|_| Err::Error(make_error(i, ErrorKind::Digit)))?;
...
#[no_mangle]
pub unsafe extern "C" fn SCFTPPortFromEprtResponse(input: *const u8, len: u32, dport_out: *mut u16) -> *mut c_char {
    if input.is_null() {
        return std::ptr::null_mut();
    }
    let buf = build_slice!(input, len as usize);
...
#[no_mangle]
pub unsafe extern "C" fn SCFTPPortFromEpsvResponse(input: *const u8, len: u32, dport_out: *mut u16) -> *mut c_char {
    if input.is_null() || dport_out.is_null() {
        return std::ptr::null_mut();
    }
    let buf = build_slice!(input, len as usize);
