...
mask <prefix-length>
  ...
  /24). Valid range: 0–32 for IPv4 prefix lengths and IPv4 bitmasks, 0–128
  for IPv6 prefix lengths. The same mask value must be used on both ``set``
...
radix trees and uses the ``ip.src`` or ``ip.dst`` sticky buffer to
obtain the raw address from a packet.
...
16 for IPv6). The CIDR dataset selects the appropriate tree
automatically based on the address length.
...
matches a stored ``192.168.1.0/24`` without any additional
configuration. ``mask`` is not valid on these commands because the
...
.. note::
   ...
   The ``memcap`` option applies to CIDR datasets. Entry cost is
   estimated at ~128 bytes per IPv4 prefix and ~192 bytes per IPv6
   prefix; once the estimated total for a family exceeds ``memcap``,
   further ``set`` operations for that family are rejected and a
   warning is logged. A ``memcap`` of 0 disables the cap. Because rules
   that use ``dataset:set`` on ``ip.src`` or ``ip.dst`` can grow the
   tree from network traffic, sizing ``memcap`` is important when
   accepting traffic from untrusted sources.
