...
typedef struct CIDRIPv4Type {
    SCRadix4Tree tree;
    SCRWLock lock;
    uint64_t bytes;  /* rough bytes consumed by stored entries, for memcap */
    uint64_t memcap; /* 0 = unlimited */
    bool memcap_warned;
} CIDRIPv4Type;
...
/* Rough per-entry cost used for memcap accounting. Actual node size varies
 * with tree topology (internal vs leaf nodes, alignment), so treat these as
 * conservative upper-bound estimates rather than exact accounting. */
